...
# input_bucket = "s3://hls-debug-output"
# output_bucket = "/Users/brad/Downloads/hls/hls_vi_sample/output"
# run_id = "brad_test_samples"
# hls_granule_id = "HLS.L30.T55GDQ.2024106T235756.v2.0"
# sr_key = f"{input_bucket}/{run_id}/{hls_granule_id}/{hls_granule_id}"
# sat_id = hls_granule_id.split(".")[1]
# /Users/brad/Downloads/hls/hls_vi_sample
...
@click.option(
    "--output_bucket",
    default="/Users/brad/Downloads/hls/hls_vi_sample/output10",
    help="Output bucket",
)


##### I don't appear to have permission to write to the bucket.
# @click.option(
#    "--output_bucket",
#    default="s3://vegetationindice/output/",
#    help="Output bucket",
# )


def main(input_bucket, run_id, hls_granule_id, output_bucket):
    """
    This script processes HLS VI data
    """
    ...
    def open_file_and_extract_metadata(base, band, band_name, data=None, metadata=None):
    ...
    def save_raster(
        ...
        - None
        """
        # scaled_data = (raster_data * 10000).astype(np.float32)
    ...
    def generate_vi_rasters(sr_ds, hls_granule_id, **extracted_attributes):
        ...
        longname_str = ", ".join(
            [f"{index}: {name}" for index, name in longname.items()]
        )
        ...
        for index_name, index_data in zip(
            ...
            index_output_path = os.path.join(output_dir, f"{fname}.{index_name}.tif")
