...
class HlsViStacItem:
    def __init(self, hls_vi_metadata, out_json, endpoint, version):
        super(self, HlsViStacItem).__init__()
        self.hls_vi_metadata = hls_vi_metadata
    ...
    def process_common_metadata(self, item, granule):
        ...
        item.common_metadata.platform = (
            granule.Platforms.Platform.ShortName.cdata.lower()
        )
        instrument = (
    ...
    def process_eo(self, item, granule):
    ...
    def add_assets(self, item, granule, endpoint, version):
        ...
        if product == "S30":
            ...
            public_url = (
                f"https://{endpoint}/lp-prod-public/HLSS30.{version}/{item_id}/"
            )
        ...
        if product == "L30":
            ...
            public_url = (
                f"https://{endpoint}/lp-prod-public/HLSL30.{version}/{item_id}/"
            )
    ...
    def process_projection(self, item, granule, band1_file):
    ...
    def process_view_geometry(self, item, granule):
    ...
    def process_scientific(self, item, granule):
    ...
    def cmr_to_item(self):
        ...
        cmr = untangle.parse(self.hls_vi_metadata)
        granule = cmr.Granule
        item_id = granule.GranuleUR.cdata
    ...
    def create_item(self):
        item = self.cmr_to_item(
            self.hls_vi_metadata, self.out_json, self.endpoint, self.version
        )
...
if __name__ == "__main__":
    ...
    output_json = "test_hls_base.json"
    main(cmr_xml, output_json, "data.lpdaac.earthdatacloud.nasa.gov", "020")
