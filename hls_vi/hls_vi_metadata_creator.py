...
- Update DataGranuleSizeInBytes - this is the HLS VI granule size:
  int(os.path.getsize(HLS_VI granule))
...
dest_tree.find("InsertTime").text = datetime.datetime.now(
    datetime.timezone.utc
).strftime(time_format)

# This needs to be updated to last update time of file
dest_tree.find("LastUpdate").text = datetime.datetime.now(
    datetime.timezone.utc
).strftime(time_format)
